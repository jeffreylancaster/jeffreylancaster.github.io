...
function writeGrid(array){
	...
	for(i=0; i<array.length; i++){
		...
		if(array[i][1] == 'h'){
			content += '<div class="stateimg"><a href="http://elections.nytimes.com/2012/primaries/states/'+array[i][0].replace(" ", "-")+'" target="_blank"><img src="images/'+array[i][0]+'.png" class="w"></a>';
			if(array[i][4] > 0){
			...
			content += '<div class="stateimg"><a href="http://elections.nytimes.com/2012/primaries/states/'+array[i][0].replace(" ", "-")+'" target="_blank"><img src="images/'+array[i][0]+'.png" class="v"></a>';
			if(array[i][4] > 0){
				content += '<center><div class="delegates_o">';
				for(k=0; k<array[i][4]; k++){
					content += '<img src="images/man_o.png" class="man">';
		...
		//for states
			...
			for(j=0; j<array[i][2].length; j++){
				...
				if(array[i][2][j][2] > 0){							
					content += '<center><div id="interactive_'+array[i][0].replace(" ", "_")+'_'+array[i][2][j][0]+'" class="graph"></div></center>';
					content += '<center><div id="hover_'+array[i][0].replace(" ", "_")+'_'+array[i][2][j][0]+'" class="hover">&nbsp;</div></center>';
					content += '<script type="text/javascript">';
					content += '$(function () {';
					content += 'var data_'+array[i][0].replace(" ", "_")+'_'+array[i][2][j][0]+' = [';
					...
					content += '];';
					content += '$.plot($("#interactive_'+array[i][0].replace(" ", "_")+'_'+array[i][2][j][0]+'"), data_'+array[i][0].replace(" ", "_")+'_'+array[i][2][j][0]+', ';
					...
					content += '$("#interactive_'+array[i][0].replace(" ", "_")+'_'+array[i][2][j][0]+'").bind("plothover", pieHover_'+array[i][0].replace(" ", "_")+'_'+array[i][2][j][0]+');';
					content += '$("#interactive_'+array[i][0].replace(" ", "_")+'_'+array[i][2][j][0]+'").bind("plotclick", pieClick_'+array[i][0].replace(" ", "_")+'_'+array[i][2][j][0]+');';
					content += '});';
					content += 'function pieHover_'+array[i][0].replace(" ", "_")+'_'+array[i][2][j][0]+'(event, pos, obj) {';
					content += 'if (!obj) return; data = obj.series.data.toString().substring(2); ';
					content += '$("#hover_'+array[i][0].replace(" ", "_")+'_'+array[i][2][j][0]+'").html(obj.series.label+": "+data);';
					content += '}';
					content += 'function pieClick_'+array[i][0].replace(" ", "_")+'_'+array[i][2][j][0]+'(event, pos, obj) {';
